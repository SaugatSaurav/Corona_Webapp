package hbv.web.database;


import hbv.web.passwordhash.hashingpassword;
...
public class DBUtil {
    ...
    private static final String URL = "jdbc:mariadb://mysql-server:3306/swe3-2023-08_db";
    private static final String USERNAME = "swe3-2023-08";
    private static final String PASSWORD = "Pu43GqxFEibPZWfRU2cA";
...
//FÜR ADMIN ---------------------------------------------------------------------------------------------------------
    ...
    public static int addUser(Connection conn, String firstname, String lastname, String address, String email, String password) throws Exception {
        ...
        try (
            ...
            preparedStatement.setString(6, hashingpassword.toHex(salt)); 
...
*/
public static int insertTimeslot(
...
*/
public static List<JSONObject> getTimeslotsForDate(
        Connection connection,
        int centerId,
        LocalDate date
) throws SQLException {
    ...
    String query =
            "SELECT z.id, z.start_time, z.end_time "
            + "FROM Zeitslot z "
            + "WHERE z.Center_id = ? "
            + "AND DATE(z.start_time) = ? "
            + "AND z.start_time > NOW() "
            + "AND NOT EXISTS ("
            + "SELECT 1 FROM buchung b "
            + "WHERE b.timeslot_id = z.id"
            + ") "
...
public static int deleteExpiredUnusedTimeslots(
        Connection connection
) throws SQLException {
    ...
    String query =
            ...
            + "FROM Zeitslot z "
            + "LEFT JOIN buchung b "
            + "ON b.timeslot_id = z.id "
            + "WHERE z.end_time <= NOW() "
            + "AND b.id IS NULL";
        ...
        try (PreparedStatement ps = con.prepareStatement(query)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    json.put("endTime", rs.getTimestamp("datum").toLocalDateTime().plusMinutes(15).toString()); 
